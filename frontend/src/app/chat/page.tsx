...
import { FiSend, FiPlus, FiCamera } from "react-icons/fi";
...
export default function ChatPage() {
  ...
  // Check auth + require complete profile
  useEffect(() => {
    (async () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (response.needs_emergency) setShowEmergency(true);
    } catch (err) {
      if (
        axios.isAxiosError(err) &&
        err.response?.status === 412 &&
        err.response?.data?.detail?.code === "profile_incomplete"
      ) {
        router.push("/onboarding?next=/chat");
        return;
      }
              ...
              {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <div className="flex gap-2 mt-6 flex-wrap justify-center">
                    ...
                    <button
                      onClick={() => router.push("/mediscan")}
                      className="text-xs bg-indigo-50 text-indigo-700 px-3 py-2 rounded-full hover:bg-indigo-100 transition-colors flex items-center gap-1"
                    >
                      <FiCamera className="w-3 h-3" /> Try MediScan (photo)
                    </button>
                  </div>
                </div>
              )}
